		...
		<div class="card-body">
			...
			<div class="table-responsive">
				<table class="table table-sm table-bordered" id="report">
					<thead>
						<tr>
							<td class="border-0 p-2" colspan="17">
								<h4 class="text-center" style="font-weight: bold; text-align: center;">DATA OF FISHING ACTIVITY</h4>
							</td>
						</tr>
						<tr>
							<td class="border-0 p-2" colspan="17">
								<h6 class="text-center" style="font-weight: bold; text-align: center;">All Surveyor</h6>
							</td>
						</tr>
						<tr class="bg-primary text-white  font-weight-bold">
							<th rowspan="2" class="align-middle text-center">No</th>
							<th rowspan="2" class="align-middle text-center">Nama Surveyor</th>
					...
					<tbody>
						...
						foreach ($user as $key => $u) {
							$lokasi = $this->db->query("SELECT * FROM `tb_lokasi` 
							INNER JOIN tb_user ON tb_lokasi.userid=tb_user.userid
							WHERE tanggal BETWEEN '$start' AND '$end' AND tb_lokasi.userid IN ('$u->userid')")->result();
							foreach ($lokasi as $l) {
